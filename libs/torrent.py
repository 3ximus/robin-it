...
class Torrent():
	...
	def __repr__(self):
		rep = "Torrent:\n"
		rep += "[name] -- %s\n" % self.name
		if self.url: rep += "[url] -- %s\n" % self.url
		if self.magnet: rep += "[magnet] -- %s\n" % self.magnet
		if self.tor_file: rep += "[tor_file] -- %s\n" % self.tor_file
		if self.size: rep += "[size] -- %s\n" % self.size
		if self.files: rep += "[files] -- %s\n" % self.files
		if self.age: rep += "[age] -- %s\n" % self.age
		if self.seeds: rep += "[seeds] -- %s\n" % self.seeds
		if self.peers: rep += "[peers] -- %s\n" % self.peers
